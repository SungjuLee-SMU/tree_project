...
Nptr Add(Nptr T, int Key, int Data)
{
	if (T == NULL)
	{
		T = new node;
		...
		T->LChild = Add(T->LChild, Key, Data);
	else
		T->RChild = Add(T->RChild, Key, Data);
	return T;
}
...
void Enqueue(Qptr PQ, int key, int Data) {
	if (PQ->Length == 0) {
		PQ->Root = Add(PQ->Root, key, Data);
		PQ->Length++;
	}
	else {
		Add(PQ->Root, key, Data);
		PQ->Length++;
	}
}
